...
sudo cp ./xfce-lock.sh /usr/local/bin/

xfconf-query -c xfce4-session -np /general/LockCommand -t string -s "/usr/local/bin/xfce-lock.sh"
...
xfconf-query -c xfce4-power-manager -np /xfce4-power-manager/lock-screen-suspend-hibernate -t bool -s true
